...
encoder_out_dim_dir = {18:512, 34:512, 50:2048, 101:2048, 152:2048}
...
class MoCo(nn.Module):
    def __init__(self, model_depth, dim=128, K=65536, m=0.999):
        ...
        self.K = K
        self.m = m
        self.encoder_q = resnet_encoder(model_depth)
        self.encoder_k = resnet_encoder(model_depth)
        self.encoder_out_dim = encoder_out_dim_dir[model_depth]
        self.encoder_q.fc = nn.Sequential(nn.Linear(self.encoder_out_dim, self.encoder_out_dim), nn.ReLU(), nn.Linear(self.encoder_out_dim, dim))
        self.encoder_k.fc = nn.Sequential(nn.Linear(self.encoder_out_dim, self.encoder_out_dim), nn.ReLU(), nn.Linear(self.encoder_out_dim, dim))
