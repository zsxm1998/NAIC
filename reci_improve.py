...
work_dir = '/nfs3-p2/zsxm/naic/preliminary/test_B/reci_improve/'
os.makedirs(work_dir, exist_ok=True)

def convert_V_to_sp(batch_size=10):
    v_hdf5_file = tables.open_file('/nfs3-p2/zsxm/naic/preliminary/test_B/k_reciprocal/V.hdf5', mode='r')
    V = v_hdf5_file.root.V
    V_sp_list = []
    for i in tqdm(range(0, V.shape[0], batch_size), desc='convert_V_to_sp'):
        V_sp_list.append(sp.csr_matrix(V[i:i+batch_size]))
    V_sp = sp.vstack(V_sp_list)
    print(V_sp.shape)
